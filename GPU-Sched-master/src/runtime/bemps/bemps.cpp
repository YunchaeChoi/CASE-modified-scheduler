...
// Added function by - yunchae
static inline void _modify_comm(bemps_shm_comm_t *comm, int64_t membytes) {
  comm->beacon.mem_B -= (long) membytes;
  comm->beacon.changed = true;
}
...
void bemps_begin(int id, int gx, int gy, int gz, int bx, int by, int bz,
  ...
  beacon.changed = 0; // initializing 'changed'
  ...
  // std::cout << "BEMPS_BEGIN TIME STAMP. tid: " << id << ", time: " <<  timeSinceEpochMillisec() << " ns\n";
  // BEMPS_STATS_LOG("tid: " << id << " , " << "BEMPS_BEGIN TIME STAMP " << timeSinceEpochMillisec() << "ns\n");
  // printf("in_bemps_begin: %ld\n", beacon.changed);
...
void bemps_free(int bemps_tid) {
  ...
  std::cout << "TIME STAMP (BEMPS_FREE). time: " << nanoseconds << " ns, "  << "tid: " << bemps_tid << "\n";
...
extern "C" {
void pre_bemps_free(int bemps_tid, long membytes) {
  int orig_beacon_q_idx;
  int free_beacon_q_idx;
  bemps_shm_comm_t *comm;
  long mem_B;
  ...
  comm->beacon.mem_B -= membytes;

  printf("in PRE BEMPS FREE - beacon.mem_B: %ld\n", mem_B);
  printf("in PRE BEMPS FREE - device_id : %d\n", device_id);
  // _reset_comm(comm);
  ...
  comm->timestamp_ns = _get_time_ns();
  comm->beacon.mem_B = mem_B - membytes;
  comm->beacon.warps = warps;
  comm->beacon.thread_blocks = thread_blocks;
  comm->sched_notif.device_id = device_id;
  comm->pid = pid;
  comm->beacon.changed = membytes;

  printf("in PRE BEMPS FREE - changed_beacon.mem_B: %ld\n", comm->beacon.mem_B);
...
int bemps_init(void) {
  ...
  BEMPS_STATS_LOG("bemps_shm.comm:      " << bemps_shm.comm << "\n");
